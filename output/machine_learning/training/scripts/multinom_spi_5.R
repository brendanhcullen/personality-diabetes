# This script applies the machine learning algorithm 'multinom' using the 'spi_5' dataset as input features
...
train_data = train_master_df %>% 
  filter(ml_model == deparse(substitute(multinom)) & spi_scoring == deparse(substitute(spi_5))) %>% 
  select(train_data) %>% 
  map_df(1)
...
filename = 'multinom_spi_5_fit.RDS'
