# This script applies the machine learning algorithm 'nnet' using the 'spi_27' dataset as input features
...
train_data = train_master_df %>% 
  filter(ml_model == deparse(substitute(nnet)) & spi_scoring == deparse(substitute(spi_27))) %>% 
  select(train_data) %>% 
  map_df(1)
...
model = train(diabetes ~ .,
              data = train_data,
              method = deparse(substitute(nnet)), 
