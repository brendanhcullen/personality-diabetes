# This script applies the machine learning algorithm 'nnet' using the 'spi_5' dataset as input features
...
train_data = train_master_df %>% 
  filter(ml_model == deparse(substitute(nnet)) & spi_scoring == deparse(substitute(spi_5))) %>% 
  select(train_data) %>% 
  map_df(1)
